...
import spring.payment.payment.adapter.out.persistent.exception.PaymentAlreadyProcessedException
            ...
            .handle { paymentOrder, sink ->
                when (paymentOrder.second) {
                    ...
                    PaymentStatus.SUCCESS.name -> {
                        sink.error(PaymentAlreadyProcessedException(PaymentStatus.SUCCESS, "이미 처리 성공한 결제입니다."))
                    }

                    PaymentStatus.FAILURE.name -> {
                        sink.error(PaymentAlreadyProcessedException(PaymentStatus.FAILURE, "이미 처리 실패한 결제입니다."))
                    }
                }
            }
            .collectList()
